...
async def delete_file(
    ...
    print("deletig file",word_count)
    print("should_update_word_count",should_update_word_count)
...
async def validate_and_upload_files(
    ...
    print("🚀 DEBUG: OKStarting validate_and_upload_files function")
    ...
    for i, file in enumerate(files):
        ...
        try:
            ...
            print("📊 DEBUG: Set status to 'Extracting'")
...
def start_training(
    ...
    print(" I Have been called - Start Training")
    ...
    if files_to_vectorize:
        logger.info(f"📄 Found {len(files_to_vectorize)} files to vectorize for bot {bot_id}")
        ...
        logger.info(f"🚫 No files pending vectorization for bot {bot_id}")
    ...
    if videos_to_vectorize:
        logger.info(f"🎯 Found {len(videos_to_vectorize)} videos to vectorize for bot {bot_id}")
        ...
        logger.info(f"🚫 No YouTube videos pending vectorization for bot {bot_id}")
    ...
    if scraped_nodes_to_vectorize:
        logger.info(f"🌐 Found {len(scraped_nodes_to_vectorize)} scraped web pages to vectorize for bot {bot_id}")
        scraped_node_ids = []
        for node in scraped_nodes_to_vectorize:
        ...
        logger.info(f"🚫 No scraped web pages pending vectorization for bot {bot_id}")
